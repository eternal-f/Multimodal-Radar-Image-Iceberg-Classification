...
def add_predict(in_put):
    train_cnn_predict = np.array([np.load(f_name) for f_name in glob('./train_predict' + '/train_predict_*')])
    test_cnn_predict = np.array([np.load(f_name) for f_name in glob('./train_predict' + '/test_predict_*')])

    n_models = train_cnn_predict.shape[0]
    seq = range(n_models)
    best_subset = seq

    for n in seq:
        for subset in combinations(seq, n + 1):
            # 计算当前子集的平均预测
            avg_predict = np.mean(train_cnn_predict[list(subset), :], axis=0)
            cand_loss = log_loss(in_put[in_put.train].is_iceberg, avg_predict)

            best_avg_predict = np.mean(train_cnn_predict[list(best_subset), :], axis=0)
            best_loss = log_loss(in_put[in_put.train].is_iceberg, best_avg_predict)

            if cand_loss < best_loss:
                best_subset = subset
                print(f"Found better subset: {best_subset}, loss: {cand_loss}")

    # 计算最佳子集的平均预测
    train_best_predict = np.mean(train_cnn_predict[list(best_subset), :], axis=0)
    test_best_predict = np.mean(test_cnn_predict[list(best_subset), :], axis=0)

    in_put.loc[in_put.train, 'cnn_predict'] = train_best_predict
    in_put.loc[~in_put.train, 'cnn_predict'] = test_best_predict
    in_put.loc[~in_put.train, 'is_iceberg'] = test_best_predict

    print(f'Best NN subset: {best_subset}')
    print(f'Best NN subset CV loss: {log_loss(in_put[in_put.train].is_iceberg, in_put[in_put.train].cnn_predict)}')
